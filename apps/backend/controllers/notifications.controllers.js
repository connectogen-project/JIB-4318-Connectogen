...
export const createNotification = async(req, res) => {
    try {
        const { notifType, message } = req.body;
        const userEmail = req.user.email; // Get email from decoded token in req.user
...
export const readNotification = async(req, res) => {
    try {
        const { id } = req.params; 
        const userEmail = req.user.email; // Get email from decoded token in req.user
        const notif = await Notification.findOne({ _id: id, userEmail });
...
export const deleteNotification = async(req, res) => {
    ...
    const userEmail = req.user.email; // Get email from decoded token in req.user
    try {    
        const result = await Notification.findOneAndDelete({ _id: id, userEmail });
        if (!result) {
